...
const routeGuard = (accessMatrix) => {
    return async (req, res, next) => {
        console.log('routeGuard', req.authInfo, req.path, accessMatrix.api.path, req.path.includes(accessMatrix.api.path));
        ...
        if (req.authInfo.groups === undefined) {
            ...
            if (req.path.includes(accessMatrix.api.path)) {
                if (accessMatrix.api.methods.includes(req.method)) {

                    let intersection = accessMatrix.api.groups
